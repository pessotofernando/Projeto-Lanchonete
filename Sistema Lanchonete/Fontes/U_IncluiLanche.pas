...
type
  TFr_IncluiLanche = class(TForm)
    ...
    procedure DBGrid1KeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure BntGravaClick(Sender: TObject);
  private
...
procedure TFr_IncluiLanche.BntGravaClick(Sender: TObject);
begin
   If Application.MessageBox('Confirma a inclusao do lanche ao Pedido?','Lanchonete',MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = IDYES Then
   begin
...
procedure TFr_IncluiLanche.DBGrid1CellClick(Column: TColumn);
begin
   if not module.LANCHES_SP_CON.active then exit;
   LANCHESINGREDIENTES(module.LANCHES_SP_CON.FieldByName('LAN_CODIGO').asinteger);
end;

procedure TFr_IncluiLanche.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if not module.LANCHES_SP_CON.active then exit;
   LANCHESINGREDIENTES(module.LANCHES_SP_CON.FieldByName('LAN_CODIGO').asinteger);
