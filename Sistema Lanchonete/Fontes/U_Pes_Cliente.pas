...
type
  ...
  public
    ...
    procedure retorna_Cliente;
    procedure limpa_campos;
  end;

var
  Fr_Pes_Cliente: TFr_Pes_Cliente;
  var_fr_tecla  : string;
...
procedure TFr_Pes_Cliente.Frame_Impressao1BntFecharClick(Sender: TObject);
begin
   close;
end;

procedure TFr_Pes_Cliente.Frame_Impressao1BntLimpaClick(Sender: TObject);
begin
   limpa_campos;
   retorna_Cliente;
end;

procedure TFr_Pes_Cliente.CodigoExit(Sender: TObject);
begin
...
procedure TFr_Pes_Cliente.retorna_Cliente;
begin

   with Module.CLIENTE_SP_SEL do
   begin
      close;
...
procedure TFr_Pes_Cliente.limpa_campos;
begin
   codigo.Text := '0';
   nome.text   := '';
...
procedure TFr_Pes_Cliente.descriExit(Sender: TObject);
begin
   retorna_Cliente;
end;

procedure TFr_Pes_Cliente.FormCreate(Sender: TObject);
begin
   Frame_Impressao1BntLimpaClick(Frame_Impressao1.BntLimpa);
end;

procedure TFr_Pes_Cliente.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFr_Pes_Cliente.Frame_Impressao1BntGravaClick(Sender: TObject);
begin

   if not Module.CLIENTE_SP_SEL.active then exit;
   if Module.CLIENTE_SP_SEL.FieldByName('CLI_CODIGO').asstring = '' then exit;

   paramentros[1] := inttostr(Module.CLIENTE_SP_SEL.fieldbyname('CLI_CODIGO').asinteger);
   paramentros[2] := Module.CLIENTE_SP_SEL.fieldbyname('CLI_DESCRI').asstring;

   close;
end;

procedure TFr_Pes_Cliente.DBGrid1DblClick(Sender: TObject);
begin
   Frame_Impressao1BntGravaClick(Frame_Impressao1.BntGrava);
end;

procedure TFr_Pes_Cliente.ApplicationEvents1Message(var Msg: tagMSG;var Handled: Boolean);
var
  i : Smallint;
begin
...
procedure TFr_Pes_Cliente.NomeExit(Sender: TObject);
begin
   retorna_Cliente;
end;

procedure TFr_Pes_Cliente.cpfExit(Sender: TObject);
begin
   retorna_Cliente;
end;

procedure TFr_Pes_Cliente.cgccpfExit(Sender: TObject);
begin
   if  cpf.EditMask = '999.999.999-99;1;_' then
   begin
      if cpf.text = '   .   .   -  ' then
      begin
         retorna_Cliente;
...
procedure TFr_Pes_Cliente.DBGrid1KeyDown(Sender: TObject;
